...
if next(SMODS.find_mod("FusionJokers")) then
    ...
    FusionJokers.fusions:register_fusion{
        jokers = {
            ...
            {name = "j_devious"},
            {name = "j_order"},
        }, cost = 8, result_joker = "j_biblio_straightemult",
        requirement = function ()
            local showman = SMODS.showman("j_biblio_straightemult")
            local has = not not next(SMODS.find_card("j_biblio_straightemult"))
...
SMODS.Joker {
    ...
    set_badges = function (self, card, badges)
        ...
        badges[#badges+1] = BIBLIO.credit_badge{type = "???", credit = "???", bcol = G.C.BLACK, tcol = G.C.WHITE}
    end,
    cost = 8,
    unlocked = true,
    discovered = false,
