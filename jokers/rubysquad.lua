...
if next(SMODS.find_mod("FusionJokers")) then
    ...
    FusionJokers.fusions:register_fusion{
        jokers = {
            ...
            {name = "j_crafty"},
            {name = "j_tribe"},
        }, cost = 8, result_joker = "j_biblio_rubysquad",
        requirement = function ()
            local showman = SMODS.showman("j_biblio_rubysquad")
            local has = not not next(SMODS.find_card("j_biblio_rubysquad"))
...
SMODS.Joker {
    ...
    set_badges = function (self, card, badges)
        ...
        badges[#badges+1] = BIBLIO.credit_badge{type = "Characters", credit = "Steven Universe", bcol = G.C.RED, tcol = G.C.WHITE}
    end,
    cost = 8,
    unlocked = true,
    discovered = false,
