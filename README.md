...
**A**: *(comically long pause for audio splicing)* 36, of which 5 are [Fusion Jokers](https://github.com/wingedcatgirl/Fusion-Jokers) exclusive and two are [Pokermon](https://github.com/InertSteak/Pokermon) exclusive *(comically long pause for audio splicing)* at the moment, unless I forgot to update this README file, which will definitely happen several times throughout the mod's lifespan.
...
**Q**: Why would I want to connect my AO3 account to a Balatro mod?  
**A**: A few Jokers care about that. If you don't connect it, they won't spawn and that's all that'll happen. ... Also "connect" might be overstating it, it just looks at the public user page like any web browser might. There's also no actual rule that it has to be _your_ AO3, and not one of those people who post seven 100-word smutty one-shots every day.

### Crossmodding if you want to
- If you want your cards to evolve with the Crucible, put the evolved form's key as a string in `biblio_evolution`. (Alternately instead of a string you can do a table of strings to select randomly, or a `function(self, card, crucible) -> string` that returns the key based on any criteria you imagine up.) For rarities you can legally do what you want but canonically it goes Common/Uncommon -> Evolved, Evolved/Rare/Epic-and-equivalent -> Ascended, Ascended/Legendary -> Exalted, plus some higher rarities that don't exist in Biblio yet.
