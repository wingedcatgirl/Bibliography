...
G.FUNCS[myprefix .. "_can_alt_use_joker"] = function(e)
    local center = e.config.ref_table.config.center
    local card = e.config.ref_table

    if not center.alt_use then
        e.config.colour = G.C.UI.BACKGROUND_INACTIVE
        e.config.button = nil
    end

    if
        (not center.can_alt_use or center:can_alt_use(e.config.ref_table)) and not e.config.ref_table.debuff
        and G.STATE ~= G.STATES.HAND_PLAYED and G.STATE ~= G.STATES.DRAW_TO_HAND and G.STATE ~= G.STATES.PLAY_TAROT
        and not (((G.play and #G.play.cards > 0) or (G.CONTROLLER.locked) or (G.GAME.STOP_USE and G.GAME.STOP_USE > 0)))
    then
        e.config.colour = e.config.bg_colour or G.C.ORANGE
        e.config.button = myprefix .. "_alt_alt_use_joker"
    else
        e.config.colour = G.C.UI.BACKGROUND_INACTIVE
        e.config.button = nil
    end
end
G.FUNCS[myprefix .. "_alt_use_joker"] = function(e)
    local int = G.TAROT_INTERRUPT
    G.TAROT_INTERRUPT = true
    local center = e.config.ref_table.config.center
    if center.alt_use then
        center:alt_use(e.config.ref_table)
    end
    e.config.ref_table:juice_up()
    G.TAROT_INTERRUPT = int
end
